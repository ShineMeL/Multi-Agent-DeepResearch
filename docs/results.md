...
- Public artifact seal verified: no (caller supplied an in-memory summary).
- `dataset_version`: not-sealed
...
| B | not reported | not reported | not reported | not reported |
| C | not reported | not reported | not reported | not reported |
| D | not reported | not reported | not reported | not reported |
...
- Bootstrap dominance proportions: not reported.
...
- Reference status: not_present.

## Human evaluation

Human rating summary: not sealed/present.

## External evaluation

External 10/20/10 benchmark results: not sealed/present.
